      ...
      <>
        <nav className="main-nav">
          ...
          <div className="nav-links">
            ...
            {/* Extra pages accessible directly */}
            {!user?.isAdmin && (
              <>
    ...
    <BrowserRouter>
      <div className="App">
        <Routes>
          ...
          <Route
            path="/about"
            element={<div className="content"><AboutPage /></div>}
          />
          <Route
            path="/register"
            element={!user ? (
          ...
          />
           ...
           <Route path="/adjuster" element={user ? (
              <AppShell>
                <AdjusterDashboard />
              </AppShell>
           ) : (<Navigate to="/login" replace />)} />
           ...
           <Route path="/alerts" element={user && user.isAdmin ? (
              <AppShell>
                <HighRiskAlerts />
              </AppShell>
           ) : (<Navigate to={user ? "/dashboard" : "/login"} replace />)} />
           <Route path="/buy-policy" element={user ? (
              <AppShell>
                <BuyPolicy />
              </AppShell>
           ) : (<Navigate to="/login" replace />)} />
           <Route path="/metrics" element={user ? (
              <AppShell>
                <WorkflowMetricsDashboard />
              </AppShell>
           ) : (<Navigate to="/login" replace />)} />
           <Route path="/overdue" element={user && user.isAdmin ? (
              <AppShell>
                <OverdueTasksReport />
              </AppShell>
           ) : (<Navigate to={user ? "/dashboard" : "/login"} replace />)} />

           {/* Help Page - Public but styled better with AppShell when logged in */}
           <Route path="/help" element={user ? (
              <AppShell>
                <HelpPage />
              </AppShell>
